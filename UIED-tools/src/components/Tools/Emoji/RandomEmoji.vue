      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
          ...
          <h2 class="text-4xl font-bold mb-3 relative inline-flex flex-col items-center">
            <div class="relative px-12">
              <span class="text-gray-800 hover:text-gray-600 transition-colors duration-300 cursor-pointer"
                @click="getNewEmoji">随机表情包</span>
            </div>
          </h2>
          <p class="text-gray-500 text-sm mt-6">每次刷新都能获取一张热门表情包</p>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-6 mb-6 relative flex justify-center items-center"
          ...
          <div class="emoji-display w-full h-full relative" v-show="emojiUrl && !loading">
            <img :src="emojiUrl" alt="随机表情包" @load="handleImageLoad"
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="border border-gray-100 rounded-lg p-6">
              <div class="flex items-center space-x-3 mb-4">
                ...
                <h4 class="text-base font-medium text-gray-900">热门表情</h4>
              </div>
        ...
        <div class="mt-12">
          ...
          <div class="space-y-4">
            <div class="border border-gray-100 rounded-lg p-6">
              <h4 class="text-base font-medium text-gray-900 mb-2">表情包的来源是什么？</h4>
              <p class="text-sm text-gray-600">
                表情包来自互联网精选素材，包含多种风格和类型，适合各种场景使用。
              </p>
            </div>
            <div class="border border-gray-100 rounded-lg p-6">
              <h4 class="text-base font-medium text-gray-900 mb-2">如何下载表情包？</h4>
              <p class="text-sm text-gray-600">
                点击"下载表情"按钮即可将当前表情包保存到本地，文件将以"表情包_时间戳.jpg"格式命名。
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="mt-8 text-sm text-gray-500">
          ...
          <p class="text-center mt-2">本工具所有内容均来源于互联网，仅供个人聊天娱乐使用。如有侵权请联系我们删除。</p>
        </div>
      </div>
    </div>
...
const getNewEmoji = async () => {
  try {
    ...
    const response = await fetch('https://api.lolimi.cn/API/bqb/api.php', {
      method: 'GET',
      headers: {
        'Accept': 'image/*'
      }
...
const downloadEmoji = async () => {
  ...
  try {
    ...
    a.href = url
    a.download = '表情包_' + new Date().getTime() + '.jpg'
