<!--
 * @file Scatter.vue
 * @description 散点图制作工具组件，支持数据可视化和图表定制
 * @author UIED技术团队
...
const info = reactive({
  title: "散点图",
  description: "在线图表制作工具，支持数据导入和样式定制"
})
    ...
    nextTick(() => {
      ...
      spreadsheet.value.change(data => {
        try {
          ...
          const newColunmData: any[] = []
          const newValueData: any[] = []

          Object.entries(rows).forEach(([index, row]: [string, any]) => {
            if (index === '0' || !row?.cells) return
            const xValue = row.cells[0]?.text
            const yValue = row.cells[1]?.text
            if (xValue && yValue) {
              newColunmData.push(Number(xValue))
              newValueData.push(Number(yValue))
            }
          })

          if (newColunmData.length > 0 && newValueData.length > 0) {
            colunmData.value = newColunmData
...
onMounted(() => {
  ...
  rowsData.value = {
    ...
    rows: {
      ...
      ...Object.fromEntries(
        colunmData.value.map((val, index) => [
          index + 1,
          {
            cells: {
              0: { text: val.toString() },
              1: { text: valueData.value[index].toString() }
            }
          }
        ])
      )
        ...
        <div>
          ...
          <div class="space-y-4">
            <div class="border-b border-gray-100 pb-4">
              ...
              <p class="text-sm text-gray-600">支持Excel格式（.xls、.xlsx）和CSV格式文件的导入，数据表格需包含两列：第一列为X轴数值，第二列为Y轴数值。</p>
            </div>
            <div class="border-b border-gray-100 pb-4">
