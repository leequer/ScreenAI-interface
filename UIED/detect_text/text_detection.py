...
def text_detection(input_file, output_file, show=False, method='google', paddle_model=None):
    '''
    ...
    print("--------7-------------"+ocr_root+"/"+name+'.png')
    visualize_texts(img, texts, shown_resize_height=800, show=show, write_path=ocr_root+"/"+name+'.png')
    print("--------8-------------")
    save_detection_json(ocr_root+"/"+name+'.json', texts, img.shape)
    print("--------9-------------")
    print("[Text Detection Completed in %.3f s] Input: %s Output: %s" % (time.perf_counter() - start, input_file, ocr_root+"/"+name+'.json'))
