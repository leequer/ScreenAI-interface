...
from similarity import CLIPScorer, clip_mae, rendered_score
...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["similarity",],
...
def find_html(response):
    # Compile the regex pattern to find the content between <html> and </html> tags
    # Using re.IGNORECASE to handle case variations and re.DOTALL to include newlines
    pattern = re.compile(r'<html\b[^>]*>.*?</html>', re.DOTALL | re.IGNORECASE)
    match = pattern.search(response)
    return match.group(0) if match else ''


def similarity_reward(completions, solution, **kwargs):
    """Reward function that checks if the completion is correct using symbolic verification, exact string matching, or fuzzy matching."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    for content, sol in zip(contents, solution):
        ...
        reward = rendered_score(find_html(content), find_html(sol), clip_mae)  
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            if reward <= 1.0:  # this condition can be changed for debug
                with open(log_path, "a", encoding='utf-8') as f:
                    f.write(f"------------- {current_time} Accuracy reward: {reward} -------------\n")
                    # f.write(f"reward_method: {accu_reward_method}\n")
                    f.write(f"image_path: {image_path}\n")
                    f.write(f"problem: {problem}\n")
                    f.write(f"Content: {content}\n")
                    f.write(f"Solution: {sol}\n")     
