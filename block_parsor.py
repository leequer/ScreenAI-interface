...
DEFAULT_IMAGE_PATH = "data/input/test5.jpg"
...
PROMPT_MERGE =''' 
你是一个专业的移动应用UI解析助手，请严格按以下要求处理用户上传的App截图：
1. ** 识别任务 **
- 提取截图中所有可见UI元素（包括按钮、文本框、图标、标签、图片等）的以下属性：
- `text`：元素内的文字内容（若无文字则为空字符串）
- `type`：组件类型（从以下选项中选择：`Button`, `Text`, `Image`, `Icon`, `InputField`, `Switch`, `Checkbox`, `Slider`, `NavigationBar`, `List`, `Card`, `Other`）
- `position`：元素位置（格式为
`{"x": 左上角横坐标, "y": 左上角纵坐标, "width": 宽度, "height": 高度}`，单位为像素）
- 忽略纯装饰性元素（如背景色块、无功能的分隔线）。

2. ** 输出格式 **
- 仅输出一个合法的JSON数组，每个元素为对象，包含
`text`, `type`, `position`
三个字段。
- 示例：
```json
[
    {
        "text": "登录",
        "type": "Button",
        "position": {"x": 120, "y": 300, "width": 135, "height": 44}
    },
    {
        "text": "用户名",
        "type": "InputField",
        "position": {"x": 40, "y": 200, "width": 295, "height": 50}
    }
]
```
...
3. ** 处理规则 **
- 坐标基准：以截图左上角为原点
`(0, 0)`，向右为x轴正方向，向下为y轴正方向。
- 文字提取：保留换行符
`\n
`，不添加额外描述。
- 组件重叠时，按视觉层级优先识别可交互元素。
- 若无法确定类型，标记为
`Other`
并尽量描述特征（如
`Unknown - 图标 + 文字`）。
...
4. ** 错误处理 **
- 若截图内容非App界面（如照片、文档），返回：`{"error": "Invalid input: Not an App screenshot"}`
- 若识别失败，在JSON中添加
`"error_detail"`
字段说明原因。'''
...
PROMPT_LIST = [
    ("header",
     "请输出header（页眉）的最小外接框。请按照 <bbox>x1 y1 x2 y2</bbox> 的格式输出边界框。请避免header中的空白区域。"),
    ("sidebar",
     "请输出sidebar（侧边栏）的最小外接框。请按照 <bbox>x1 y1 x2 y2</bbox> 的格式输出边界框。请避免sidebar中无意义的空白区域。"),
    ("navigation",
     "请输出navigation（导航栏）的最小外接框。请按照 <bbox>x1 y1 x2 y2</bbox> 的格式输出边界框。请避免navigation中的空白区域。"),
    ("main content",
     "请输出main content（主内容区）的最小外接框。请按照 <bbox>x1 y1 x2 y2</bbox> 的格式输出边界框。请避免main content中的空白区域。"),
]
...
if __name__ == "__main__":
    ...
    client = GPT(api_path)  # Change your models according to your needs: Qwen(api_path), GPT(api_path), Gemini(api_path)
